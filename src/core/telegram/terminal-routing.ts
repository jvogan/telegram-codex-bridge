...
const TERMINAL_PRIMARY_BRIDGE_PATTERN = /\b(?:make|generate|create|draw|diagram)\s+(?:an?\s+)?(?:image|picture|photo|diagram|art|illustration|visual)\b|\b(?:send|reply)\s+(?:with\s+)?(?:voice|audio|voice\s+note)\b|\b(?:voice|audio)\s+update\b|\b(?:speak|tts|text\s*to\s*speech|transcribe|asr|voice\s+note)\b|\b(?:web\s*search|search\s+(?:the\s+)?web|search\s+online|look\s+online|internet\s+search|browser\s+search|find\s+(?:a\s+)?(?:paper|article|source|citation)\s+online)\b|\b(?:(?:call|live\s+call)\s+(?:status|arm|disarm|hangup|hang\s+up|invite)|(?:status|arm|disarm|hangup|hang\s+up)\s+(?:the\s+)?(?:call|live\s+call))\b|\b(?:call\s+me|live\s+call|start\s+(?:a\s+)?call|mini\s*call|desktop\s+control|screen\s+share|screenshot|click|type\s+into|launch\s+(?:the\s+)?(?:app|browser|window)|open\s+(?:the\s+)?(?:app|browser|window))\b/i;
...
export function terminalConversationBlocker(text: string, options: { allowWorkspaceWrites: boolean }): string | null {
  ...
  if (isTerminalPrimaryBridgeRequest(text)) {
    return "Terminal chat mode leaves native image/audio, live-call, web-search, and desktop-control work on the bound desktop bridge path.";
  }
