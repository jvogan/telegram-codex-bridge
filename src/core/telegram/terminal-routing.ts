...
const TERMINAL_READONLY_TARGET_PATTERN = /\b(?:download|downloads|desktop|documents?|local\s+(?:file|folder|pdf|slides?)|pdf|slides?|pptx?|docx?|xlsx?|csv|log|logs?|repo|repository|workspace|folder|directory|file)\b/i;
const TERMINAL_READONLY_ACTION_PATTERN = /\b(?:read|summari[sz]e|inspect|review|check|find|list|locate|look\s+at|analy[sz]e|extract|compare|search|grep|cat|tail|head|status|diagnose)\b/i;
...
const TERMINAL_UNSAFE_PATTERN = /\b(?:edit|modify|change|fix|implement|refactor|delete|remove|rename|move|commit|merge|checkout|reset|install|deploy|publish|write\s+(?:to|in)|save\s+(?:to|in)\s+(?:the\s+)?(?:repo|repository|workspace)|(?:npm|pnpm|yarn)\s+(?:i|install|add)|git\s+(?:commit|push|pull|checkout|reset|merge)|secret|token|api\s*key|password|credential|\.env|click|type\s+into|desktop\s+control|screen\s+share|call\s+me|live\s+call|voice\s+reply|reply\s+with\s+audio|make\s+(?:an?\s+)?image|generate\s+(?:an?\s+)?image|draw)\b/i;
...
const TERMINAL_PRIMARY_BRIDGE_PATTERN = /\b(?:make|generate|create|draw|diagram)\s+(?:an?\s+)?(?:image|picture|photo|diagram|art|illustration|visual)\b|\b(?:send|reply)\s+(?:with\s+)?(?:voice|audio|voice\s+note)\b|\b(?:voice|audio)\s+update\b|\b(?:speak|tts|text\s*to\s*speech|transcribe|asr|voice\s+note)\b|\b(?:web\s*search|search\s+(?:the\s+)?web|search\s+online|look\s+online|internet\s+search|browser\s+search|find\s+(?:a\s+)?(?:paper|article|source|citation)\s+online)\b|\b(?:call\s+me|live\s+call|start\s+(?:a\s+)?call|mini\s*call|desktop\s+control|screen\s+share|screenshot|click|type\s+into|launch\s+(?:the\s+)?(?:app|browser|window)|open\s+(?:the\s+)?(?:app|browser|window))\b/i;
...
function normalizedTaskText(task: QueuedTelegramTask): string {
...
export function isTerminalPrimaryBridgeRequest(text: string): boolean {
  return TERMINAL_PRIMARY_BRIDGE_PATTERN.test(textWithoutSafetyProhibitions(text));
}
...
export function buildTerminalPromptForTask(task: QueuedTelegramTask): string {
  ...
  return buildTerminalPromptForText(task.text);
}
...
export function buildTerminalConversationPromptForTask(task: QueuedTelegramTask, options: { allowWorkspaceWrites: boolean }): string {
  ...
  return buildTerminalConversationPromptForText(task.text, options);
}
